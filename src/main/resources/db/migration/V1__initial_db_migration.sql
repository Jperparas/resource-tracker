...
(
    ...
    pca_serial_number VARCHAR(45)      NOT NULL,
    spot_id           INT              NOT NULL,
    power_type_id     TINYINT UNSIGNED NOT NULL,
    resource_level    TINYINT UNSIGNED NOT NULL,
    PRIMARY KEY (pca_id),
    UNIQUE INDEX pca_serial_number_UNIQUE (pca_serial_number ASC) VISIBLE,
    INDEX fk_pcas_power_types_idx (power_type_id ASC) VISIBLE,
    INDEX fk_pcas_spots1_idx (spot_id ASC) VISIBLE,
    CONSTRAINT fk_pcas_power_types
        FOREIGN KEY (power_type_id)
            REFERENCES power_types (power_type_id)
            ON DELETE NO ACTION
            ON UPDATE CASCADE,
    CONSTRAINT fk_pcas_spots
        FOREIGN KEY (spot_id)
            REFERENCES spots (spot_id)
            ON DELETE NO ACTION
            ON UPDATE CASCADE
